...
def is_all_caps(s):
    ...
    for char in s:
        ...
        # check if a single character in this string is a capitalized letter
        if (is_capitalized(char)):
            n += 1
        else:
            return False

    # check if this string only contains a single latter
    if (n >= 2):
        return True
    else:
        return False
    ...
    if is_all_caps(word):
        if len(word) > 1:
            for x in range(len(word)):
                a += char_to_braille(word[x]) + '\n\n'
            a = CAPITAL + '\n\n' + CAPITAL + '\n\n' + a
        else: # when the string only contains one capitalized letter
            a += CAPITAL + '\n\n' + char_to_braille(word[i]) + '\n\n'
        return a
    else:
        for i in range(len(word)):
            if word[i] in DIGITS: # put NUMBER in front of digits
                a += NUMBER + '\n\n' + char_to_braille(word[i]) + '\n\n'
            elif is_capitalized(word[i]): # put CAPITAL in front of capital letter
                a += CAPITAL + '\n\n' + char_to_braille(word[i]) + '\n\n'
            else:
                a += char_to_braille(word[i]) + '\n\n'
        return a
...
def new_filename(fname, addition):
    ...
    '''
    new_file = fname[:-4] + '_' +addition + fname[-4:]
    return new_file
