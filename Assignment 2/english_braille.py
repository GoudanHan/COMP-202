...
def convert_quotes(text):
    ...
    '''
    # ADD CODE HERE
    # use replace() to replace the straight quotation marks
...
def convert_english_numbers(text):
    ''' (str) -> str
    this function returns the English version text which contains numbers.
    >>> convert_english_numbers('COMP202')
    'COMPbjb'
    '''
    # use global variable numbers_alphabet to convert the digit into unicode
    # without adding other unicodes in it
    
    ### I don't know how to add ⠼ and ⠰ in front of and at the end of the digits
    ### So the doctest is gonna fail
    
    for i in range(len(text)):
        if text[i] in DIGITS:
            text = text.replace(text[i], numbers_alphabet[int(text[i])])
    return text 

def convert_english_punctuations(text):
    ''' (str) -> str
    this function returns the English version text which contains punctuation.
    >>> convert_english_punctuations('hi?')
    'hi('
    >>> convert_english_punctuations('“hi”')
    '(hi)'
    >>> convert_english_punctuations('(hi)')
    '"hi"'
    '''
    # use string.replace to replace some of the punctuations into English version
    text = text.replace('(', '\"')
    text = text.replace(')', '\"')
    text = text.replace('“', '(')
    text = text.replace('”', ')')
...
def english_text_to_braille(text):
    ...
    # You may want to put code after this comment. You can also delete this comment.
    text = convert_english_numbers(text)
    text = convert_english_punctuations(text)
